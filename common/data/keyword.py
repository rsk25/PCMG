...
class Keyword(TypeBatchable):
    #: Tokenized keywords. Shape: [N, D].
    keywords: Label
    ...
    def device(self) -> torch.device:
        return self.keywords.device

    @classmethod
    def from_dict(cls, raw: dict, n_numbers: int, var_list: List[str], tokenizer, field: str = 'explanations') -> 'Keyword':
        ...
        return Keyword(numbers=numbers, variables=variables, worker=worker_index)

    @classmethod
    def from_tensors(cls, numbers: torch.LongTensor, variables: torch.Tensor, n_numbers: int) -> 'Keyword':
        # Truncate padded positions
        numbers = Label(numbers[:n_numbers])
        variables = Label(variables)

        return Keyword([numbers], [variables], worker=0)

    def as_dict(self) -> dict:
        return dict(numbers=self.numbers, variables=self.variables, worker=self.worker)

    def to(self, *args, **kwargs) -> 'Keyword':
        return move_to(self, *args, **kwargs)

    def smoothed_cross_entropy(self, numbers: Prediction, variables: Prediction,
                               smoothing: float = 0.1) -> Dict[str, torch.Tensor]:
        losses = []
    ...
    def extends_to(self, next_number: torch.LongTensor, next_variable: torch.LongTensor) -> 'Keyword':
        return Keyword(numbers=[num.extends_to(next_number) for num in self.numbers],

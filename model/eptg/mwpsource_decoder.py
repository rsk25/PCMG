...
from common.data import Encoded, Label, Text
from model.base.chkpt import *

# MWPSource = promt + keyword embeddings + equation embeddings
class MWPSourceDecoder(CheckpointingModule):
    """
    Base model for prompt generation/classification (Abstract class)
    """
    ...
    def __init__(self, encoder: str = DEF_ENCODER, **kwargs):
        ...
        # Register prefix and infix for generating MWP 
        # Shape [P]
        self.register_buffer('_prefix_for_kw', torch.LongTensor(tokenizer.encode('generate math word problem with:')[:-1])) # exclude [SEP] at the end
        # self.register_buffer('_infix_for_eq', torch.LongTensor(tokenizer.encode(', and equation:')[1:-1])) # exclude [CLS] at the beginning and [SEP] at the end


    @property
    def prefix_length(self) -> int:
        return self._prefix_for_kw.shape[0]

    @property
    def infix_length(self) -> int:
        return self._infix_for_eq.shape[0]

    def forward(self, text: Encoded, text_label: Label, keywords: Label, **kwargs) -> Tuple[Encoded, Encoded, Optional[tuple], int]:
        # text: [B,S]
        # target: [B,D]
        # out: [B,D]
        ...
        word_emb, full_emb, prefix_len = self.build_decoder_input(keywords, text_label.equation, text_label)
    ...
    def build_decoder_input(self, keywords, equation, target, prefix: Label = None):
        # Add prefix
        # We want: "explain a number <VECTOR VALUES> <TARGET TOKENS>"
        # Concatenate prefix and snippet labels.  [P] + [B, T] -> [B, P+T]
        context_input = equation.prepend(keywords)
        context_input = context_input.prepend(self._prefix_mwp_gen)

...
class MultiheadAttentionWeights(nn.Module):
    ...
    def hidden_dim(self) -> int:
        ...
        """
        return self.config.get(MDL_Q_HIDDEN, DEF_Q_HIDDEN)
    ...
    def num_heads(self) -> int:
        ...
        """
        return self.config.get(MDL_Q_HEAD, DEF_Q_HEAD)
...
class TransformerLayer(nn.Module):
    ...
    def __init__(self, hidden_dim: int = 768, intermediate_dim: int = 2048,
        ...
        if not self.cross_only:
            ...
            self.attn = MultiheadAttention(**{MDL_Q_HIDDEN: hidden_dim, MDL_Q_HEAD: num_attention_heads,
                                              MDL_Q_LN_EPS: layernorm_eps})
        # Source-Target attention layer
        self.mem = MultiheadAttention(**{MDL_Q_HIDDEN: hidden_dim, MDL_Q_HEAD: num_attention_heads,
                                         MDL_Q_LN_EPS: layernorm_eps})
        ...
        if not self.cross_only:
            ...
            self.norm_attn = MaskedLayerNorm(hidden_dim, eps=layernorm_eps)
        # Post Layer Normalization for source-target attention
        self.norm_mem = MaskedLayerNorm(hidden_dim, eps=layernorm_eps)
        # Post Layer Normalization for outputting
        self.norm_out = MaskedLayerNorm(hidden_dim, eps=layernorm_eps)
    ...
    def forward(self, target, target_ignorance_mask=None, target_attention_mask=None,
        ...
        if is_cached:
            # LayerNorm mask should be [B, 1], with the last entry
            norm_mask = target_ignorance_mask[:, -1:]
        else:
            norm_mask = target_ignorance_mask
        ...
        if not self.cross_only:
            ...
            target = self.norm_attn(target, ignorance_mask=norm_mask)
        ...
        if memory is not None:
            ...
            target = target + attented
            target = self.norm_mem(target, ignorance_mask=norm_mask)
        ...
        target = self.norm_out(target, ignorance_mask=norm_mask)
